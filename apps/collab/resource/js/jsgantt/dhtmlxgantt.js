 ...
 *  @topic: 0
 */
function GanttProjectInfo(id, name, startDate, color)
{
    this.Id = id;
    this.Name = name;
...
GanttChart.prototype.doLoadDetails = function(isLocal)
{
    ...
    for (var j = 0; j < projectArr.length; j++)
    {
        ...
        var color = projectArr[j].getAttribute("color");
        var startDate = startDateTemp.split(",");
        var project = new GanttProjectInfo(projectArr[j].getAttribute("id"), projectArr[j].getAttribute("name"), new Date(startDate[0], (parseInt(startDate[1]) - 1), startDate[2]), color);
        ...
        for (var i = 0; i < taskArr.length; i++) {
            ...
            color = (this.xmlLoader.doXPath("./color", taskArr[i])[0].firstChild == null) ? "" : this.xmlLoader.doXPath("./color", taskArr[i])[0].firstChild.nodeValue;
            var task = new GanttTaskInfo(id, name, new Date(est[0], (parseInt(est[1]) - 1), est[2]), duration, percentCompleted, predecessorTaskId, color);
...
GanttChart.prototype.readChildTasksXML = function(parentTask, childTasksArrXML)
{
    ...
    for (var i = 0; i < childTasksArrXML.length; i ++)
    {
        ...
        color = (this.xmlLoader.doXPath("./color", childTasksArrXML[i])[0].firstChild == null) ? "" : this.xmlLoader.doXPath("./color", childTasksArrXML[i])[0].firstChild.nodeValue;
        var task = new GanttTaskInfo(id, name, new Date(est[0], (parseInt(est[1]) - 1), est[2]), duration, percentCompleted, predecessorTaskId,color);
        task.ParentTask = parentTask;
...
GanttChart.prototype.printToWindow = function(message)
{
    ...
    o.print();
    if (message!==null) {
        //setTimeout('o.alert(\''+ (message ? message : "Use browser\\\'s menu \"File->Print preview\" to setup page layout.") +'\');',1000);
    }
};
...
GanttTask.prototype.getPopUpInfo = function(object, event)
{
    ...
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>EST:&nbsp;</span><span class='ut'>" + this.TaskInfo.EST.getFullYear() + "-" + (this.TaskInfo.EST.getMonth() + 1) + "-" + this.TaskInfo.EST.getDate() + "</span><br/>";
...
GanttProject.prototype.resizeProjectItem = function(width)
{
    ...
    if (percentCompleted > 0 && percentCompleted < 100)
    {
        ...
        try{
        	this.projectItem[0].firstChild.rows[0].cells[0].childNodes[0].firstChild.style.width = Math.round(width * percentCompleted / 100) + "px";        	
        }catch(e){}
...
GanttTask.prototype.resizeTaskItem = function(width)
{
    ...
    if (c)
    {
        ...
        try{
        	c.childNodes[0].firstChild.style.width = parseInt(c.width) * width / 100 + "px";
        }catch(e){};
    }
...
GanttProject.prototype.createProjectItem = function()
{
    ...
    if (this.percentCompleted != -1)
    {
        if (this.percentCompleted != 0)
        {
            ...
            colorMask.style.backgroundColor='#'+this.Project.Color;
            colorMask.style.width = (this.percentCompleted * this.Duration * this.Chart.hourInPixelsWork) / 100 + "px";
            colorMask.style.height = this.Chart.heightTaskItem + "px";
            colorMask.className = 'progressBar';
            
            colorMask.appendChild(imgPr);
            cellprojectItem.appendChild(colorMask);
        ...
        if (this.percentCompleted != 100)
        {
            ...
            imgPr.style.height = this.Chart.heightTaskItem + "px";
            cellprojectItem.appendChild(imgPr);
            imgPr.src = this.Chart.imgs + "parentnode_progress_bg.png";
        }
...
GanttTask.prototype.createTaskItem = function()
{
    ...
    tblTaskItem.id="task_"+this.TaskInfo.Id;
    ...
    if (this.TaskInfo.PercentCompleted != 0)
    {
        ...
        progressBar.style.width = (this.TaskInfo.PercentCompleted * this.TaskInfo.Duration * this.Chart.hourInPixelsWork) / 100 + "px";
        progressBar.style.height = this.Chart.heightTaskItem + "px";
        
        var colorMask = document.createElement("div");
        colorMask.style.backgroundColor='#'+this.TaskInfo.color;
        colorMask.style.width = (this.TaskInfo.PercentCompleted * this.TaskInfo.Duration * this.Chart.hourInPixelsWork) / 100 + "px";
        colorMask.style.height = this.Chart.heightTaskItem + "px";
        colorMask.className = 'progressBar';
        ...
        imgPr.style.height = this.Chart.heightTaskItem + "px";
        //imgPr.style.position = 'absolute';
        
        colorMask.appendChild(imgPr);
        cellTblTask.appendChild(colorMask);
    ...
    var taskDblClick = function() {
        self.Chart.callEvent("onTaskDblClick", [self]);
    };
    this.addEvent(divMove, 'dblclick', taskDblClick, false);    
    ...
    if (this.showObjectInfo)
    {
        if (this.object) {
            if (this.object.constructor == GanttTask) {
                ...
                this.insertData(t, "EST",this.object.TaskInfo.EST.getFullYear())+ "/" + (this.object.TaskInfo.EST.getMonth() + 1) + "/" +  this.object.TaskInfo.EST.getDate() ;
            ...
            } else
            {
                ...
                this.insertData(t, "Start date", this.object.Project.StartDate.getFullYear()) + "/" + (this.object.Project.StartDate.getMonth() + 1) + "/" + this.object.Project.StartDate.getDate();
            }
        }
    }
