(function(jQuery) {
	...
	jQuery.fn.stickyNotes.deleteNote = function(delete_button) {	
		if( parent_id = jQuery(delete_button).parent().attr("id") ){
	...
	jQuery.fn.stickyNotes.renderNote = function(note) {
		...
		var _div_date 	= 	jQuery(document.createElement('div')).addClass('jStickyDate').html('<span>'+note.create_time+'</span>');
		var _div_share 	= 	jQuery(document.createElement('div')).addClass('jStickyShare').html(  note.elemShare );
    ...
    jQuery.fn.stickyNotes.createNoteBackground = function(note) {
        var background = null;
    	if(!note){
    		bgcolor = '#ffcc00';
    	}else{
