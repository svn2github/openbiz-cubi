...
class WorklogReportStatDO extends BizDataObj
{
	...
	{		
		...
		{
			$create_date = date("Y-m-d",strtotime($row['create_time']));
			$users_count = BizSystem::getObject("project.statistics.do.WorklogReportUserCountDO")->directfetch($this->m_SearchRule)->count();
			if($users_count>0){
		...
		$prtForm = BizSystem::getObject("project.statistics.form.UserWorkhourReportForm");
