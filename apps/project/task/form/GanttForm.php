...
class GanttForm extends ChangeLogForm
{
    ...
    public function earliestProjectId($ids)
    {
		...
		$projects = BizSystem::getObject("project.project.do.ProjectDO",1)->directfetch($searchRule,0,0,"[start_time] ASC");
	...
	public function renderGantt()
	{
		...
		foreach($proj_ids as $default_project_id)
		{
			...
			if($default_project_id)
			{
				...
				$projectData = BizSystem::getObject("project.project.do.ProjectDO",1)->fetchById($projectRec['project_id']); 
			...
			if($recList->count() && !isset($projectRec['project_start_time']))
			{					
