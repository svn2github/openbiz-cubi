...
// helper function here
    function displayElement($objName, $elem, $level=0)
    {
        ...
        if ($level < 4)  // only expand first n levels
            $class = 'open';
        else {
		...
		if($elem->m_Type=='report')
		{
		    if ($elem->m_Id == $report_id) $clicked="clicked";
		    else $clicked = '';
		    $class = 'leaf';
			echo "<li id=\"".$objName."_"."$nodeId\" rel=\"".$elem->m_Type."\" value=\"$nodeId\" class=\"$class\"><a style=\"width:100px;height:auto;display:inline-block;table-layout:fixed\"  class=\"leaf2 $clicked\" href=\"".$elem->m_Id."\">$text</a></li>\n";
		}elseif($elem->m_Type=='folder' && count($children)>0){        
			if ($children && count($children)>0)
			{
			    if(can_display($elem)){
   ...
   function can_display($elem){
		$children = $elem->m_ChildNodes;
		if ($children && count($children)>0)
		{
			foreach ($children as $chld)
			{
				if($chld->m_Type=='report'){
					return true;
				}else{
					if( can_display($chld) ==true){
						return true;
					}
				}
			}
