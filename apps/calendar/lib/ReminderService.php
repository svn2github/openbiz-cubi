...
class ReminderService
{
	...
	protected function CheckRemidforTask(){
		 ...
		 */
		$taskDO = BizSystem::getObject("project.task.do.TaskSystemDO",1);
		$searchRule = "(
						    [reminder_status]=0 
						AND [reminder]=1 
						AND [status] = 0 
	...
	protected function CheckRemidforEvent(){
		 ...
		 * select Now(), DATE_ADD(NOW(), INTERVAL -15 MINUTE)
		 */
		$taskDO = BizSystem::getObject("calendar.do.EventSystemDO",1);
		$searchRule = "(
						    [reminder_status]=0 
						AND [reminder]=1 
	...
	protected function CheckNotifyforEvent(){
		...
		foreach ($taskList as $taskRec)
		{
			 ...
			 */
			if($taskRec['recurrence']==0){
				$contactList = BizSystem::getObject("calendar.do.EventContactDO")->directFetch("[event_id]='".$taskRec['Id']."'");
	...
	public function ClearTaskReminderStatus($taskDO)
	{
		...
		if($reminder_prev!=$reminder_new){
			$rec = BizSystem::getObject("task.do.TaskSystemDO",1)->fetchByID($Id);
	...
	public function ClearEventReminderStatus($eventDO)
	{
		...
		if($reminder_prev!=$reminder_new || $recurrence_prev!=$recurrence_new){
			$rec = BizSystem::getObject("calendar.do.EventSystemDO",1)->fetchByID($Id);
			if($rec){
				$rec->reminder_status=0;
