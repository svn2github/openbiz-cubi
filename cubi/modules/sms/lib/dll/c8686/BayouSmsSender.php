...
class BayouSmsSender
{
    ...
    var $use_curl                    = false;
	//调用例子，在最下面，仅供参考调用
	public function sendsms($username,$password,$mobiles,$msg,$schedule)
	{
		...
		{			
			...
			$balance = $matches[1];
		  	$result=array('status'=>1,'msg'=>'成功',"balance"=>$balance);
		  return $result;
		}
		else
		{
			preg_match("/errordescription\>(.*?)\<\/errordescription/si",$ret,$matches);			
			$result=array('status'=>0,'msg'=>$matches[1]);
		 return $result;
		}
	}
