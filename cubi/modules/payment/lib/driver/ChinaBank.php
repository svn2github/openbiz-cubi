...
class ChinaBank extends PaymentAdapter
{
	...
	public function GetReturnData(){
		...
		$data['buyer_account'] 	= iconv("GB2312","UTF-8",trim($_REQUEST['v_pmode']));
		$data['buyer_id']	 	= iconv("GB2312","UTF-8",trim($_REQUEST['v_pmode']));
		$data['order_id'] 		= trim($_REQUEST['v_oid']);
		$data['trans_id'] 		= trim($_REQUEST['v_idx']);
		$data['txn_id'] 		= trim($_REQUEST['v_oid']);
		$data['subject'] 		= iconv("GB2312","UTF-8",trim($_REQUEST['v_pmode']));
		$data['amount'] 		= trim($_REQUEST['v_amount']);
		$data['status'] 		= trim($_REQUEST['v_pstatus']);
		$data['custom'] 		= trim($_REQUEST['remark1']);
