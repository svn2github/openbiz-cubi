...
class apiOAuth2 extends apiAuth {
   ...
   */
  public function authenticate($service,$obj=null) {
    if (isset($_GET['code'])) {
	  $redirectUri= $this->redirectUri?$this->redirectUri:$obj->setRedirectUri;
	  $clientId= $this->clientId?$this->clientId:$obj->setClientId;
	  $clientSecret= $this->clientSecret?$this->clientSecret:$obj->setClientSecret;
      // We got here from the redirect from a successful authorization grant, fetch the access token
      $request = apiClient::$io->makeRequest(new apiHttpRequest(self::OAUTH2_TOKEN_URI, 'POST', array(),
	  array(
          'code' => $_GET['code'],
   ...
   * @return string
   */
  public function createAuthUrl($scope,$obj=null) {
	 
    if($obj)
	{
		$params = array(
			'response_type=code',
			'redirect_uri=' . urlencode($obj->setRedirectUri),
			'client_id=' . urlencode($obj->setClientId),
